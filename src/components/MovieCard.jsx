...
//     localStorage.setItem(`favorite_${movie.id}`, !isFavorite);
//   }
...
const MovieCard = ({ movie }) => {
  ...
  useEffect(() => {
    const storedValue = localStorage.getItem(`favorite_${movie.id}`);
    if (storedValue !== null) {
  ...
  const toggleFavorite = () => {
    ...
    // ✅ Save as string
    localStorage.setItem(`favorite_${movie.id}`, newValue);
  };
